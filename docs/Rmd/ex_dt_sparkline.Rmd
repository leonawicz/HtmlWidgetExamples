...
##
##
## Examples using historical climate data

In the examples below, I use a 1950 - 2009 subset of SNAP's 2-km resolution downscaled CRU 3.2 temperature and precipitation data for Edmonton, Alberta and Anchorage, Alaska
...
Methods for filtering differ based upon the data structure. The location, climate variable, and month columns represent factors. Filtering allows you to select specific levels.
If they were treated as character strings, the filter method would be to type in the box and rows would filter to unique matches, but this would not allow you to select multiple values.
It would be a good method if you want to only permit filtering to a single string value or some subset that are clearly related by name.
I left the decades column as character strings as an example. Years can be filtered using a slider.

Sorting behavior is alphabetical for factors as well as character strings. On the javascript side, factor information in **R** appears to be lost as far as sorting is concerned, despite the unique factor filtering behavior available.
Months have factor levels with an obvious ordering, unlike locations or climate variables. At most you might wish to reverse their default order.
If you sort by month it will change to alphabetical order. Sorting again will lead to reverse alphabetical order.
The original, sensible ordering cannot be recovered without reloading the page.

In the remainder of the examples, the location and climate variables are restrcited to Edmonton, Alberta temperature for simplicity.
...
In-line graphs can be shown with calls in your `R` Markdown document in the form `r rinline("sparkline(dat.p)")`.
Different plots can be made by passing a `type` argument, e.g., `r rinline("sparkline(dat.p, type='bar')")`.
Options include but are not limited to line graphs `r sparkline(dat.p)`, bar graphs `r sparkline(dat.p, type='bar')`, and box plots `r sparkline(dat.p, type='box')`.
