...
In-line graphs can be shown with calls in your Rmarkdown document in the form ``r sparkline(fai.p)``.
...
I make four data tables with sparklines, using lines, bars, and two with boxplots.
...
Sparklines are msot often used in-line among text, though in-line in a table is really no different and can in fact only make them easier to juxtapose and compare.
...
but I don't need to add these columns because I can hover over the boxplot and see those values in the popover. I can also see information about outliers.
...
Other information like the distribution of a random variable, a trend line, an expression of variability around a trend, or some other functional form, can be most quickly interpeted from a visual description.
